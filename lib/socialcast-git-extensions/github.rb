  ...
  module Github
    def create_pull_request(username, password, branch, repo, body)
      ...
      begin
        ...
        response = RestClient::Request.new(:url => "https://api.github.com/repos/#{repo}/pulls", :method => "POST", :user => username, :password => password, :payload => payload, :headers => {:accept => :json, :content_type => :json}, :proxy => ENV['HTTPS_PROXY']).execute
